...
//http://localhost:8888/Tokuppee-Portfolio/php/new_menber.php
session_start();
...
if (!empty($_POST)) { //まだ何も入力していない状態
    ...
    if (empty($error)) {
        $member = $db->prepare('SELECT COUNT(*) AS cnt FROM menbers WHERE name=?');
    ...
    if (empty($error)) { //エラーという変数がなければ
        ...
        $stm = $db->prepare('INSERT INTO menbers SET name=?,  password=?, created=NOW()');
        $stm->execute(array(
            $_POST['name'],
            sha1($_POST['pass']),
        ));
...
<body>
    ...
    <form action="" method="post">
        ...
        <input type="submit" value="登録" class="form_btn" />
